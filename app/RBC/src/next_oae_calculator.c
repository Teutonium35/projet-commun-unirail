...
position_t next_eoa(int num_train, position_t *pos_trains, int next_balise_avant_ressource, const int *chemins, const int *len_chemins){
    int i = 0; 
    position_t current_position = pos_trains[num_train];
    int chemin[] = chemin[num_train];
    ...
    if(next_balise_avant_ressource == chemin[num_balise_check] && current_position.pos_r <= 0) return({next_balise_avant_ressource, 0.0});
    ...
    while(1):
        ...
        if(any(objectif_surbalise)){
            float min_pos_r = 10000000000000000000;
            for(int j = 0, j<=2, j++){
...
int main(){
    ...
    int next_balise_avant_ressources = 2;
    position_t EOA = next_eoa(um_train,pos_trains,next_balise_avant_ressource, chemins, len_chemins);
