...
void MESCAN_SetBitHighNoVerifID(uCAN1_MSG *CanMsg, MC_Bit DefBit) {
    uint8_t valData;
    valData = CanMsg->array[OFFSET_DATA0_TO_7 + DefBit.IndiceData];
    CanMsg->array[OFFSET_DATA0_TO_7 + DefBit.IndiceData] = valData | (1 << DefBit.IndiceBit);
}
...
void MESCAN_SetBitLowNoVerifID(uCAN1_MSG *CanMsg, MC_Bit DefBit) {
    uint8_t valData;
    valData = CanMsg->array[OFFSET_DATA0_TO_7 + DefBit.IndiceData];
    CanMsg->array[OFFSET_DATA0_TO_7 + DefBit.IndiceData] = valData & (~(1 << DefBit.IndiceBit));
}
...
void MESCAN_SetBitNoVerifID(uCAN1_MSG *CanMsg, MC_Bit DefBit, bool valbit) {
    uint8_t valData;
    valData = CanMsg->array[OFFSET_DATA0_TO_7 + DefBit.IndiceData];
    if (valbit)
        CanMsg->array[OFFSET_DATA0_TO_7 + DefBit.IndiceData] |= valData | (1 << DefBit.IndiceBit);
...
bool MESCAN_GetBitNoVerifID(uCAN1_MSG *CanMsg, MC_Bit DefBit) {
    uint8_t valData;
    valData = CanMsg->array[OFFSET_DATA0_TO_7 + DefBit.IndiceData] & (1 << DefBit.IndiceBit);
    if (valData)
        return true;
...
uint8_t MESCAN_GetData8NoVerifID(uCAN1_MSG *CanMsg, MC_Data8 DefData) {
    uint8_t valData = 0;
    valData = CanMsg->array[OFFSET_DATA0_TO_7 + DefData.IndiceData];
    return valData;
}
...
void MESCAN_SetData8NoVerifID(uCAN1_MSG *CanMsg, MC_Data8 DefData, uint8_t Data) {
...
////////////////////////////////////////////////////////////////////////////////
//Ajout pour train ECL
////////////////////////////////////////////////////////////////////////////////

bool CompareDataMesCANandTab8(uCAN1_MSG * MesCAN, uint8_t* dataArray, uint8_t lenTab) {
    int16_t i;
    bool isMesCANidentique = true;
    if (lenTab > 8)
        lenTab = 8; //MAX_DLC = 8

    for (i = 0; i < lenTab; i++) {
        if (MesCAN->array[i + OFFSET_DATA0_TO_7 ] != dataArray[i])
            isMesCANidentique = false;
    }
    return (isMesCANidentique);
}

void CopyDataMesCAN_ToTab(uCAN1_MSG * MesCAN, uint8_t* dataArray, uint8_t lenTab) {
    int16_t i;
    if (lenTab > 8)
        lenTab = 8; //MAX_DLC = 8

    for (i = 0; i < lenTab; i++) {
        dataArray[i] = MesCAN->array[i + OFFSET_DATA0_TO_7 ];
    }
    return;
}
////////////////////////////////////////////////////////////////////////////////
//Fin de l'ajout pour train ECL
////////////////////////////////////////////////////////////////////////////////
